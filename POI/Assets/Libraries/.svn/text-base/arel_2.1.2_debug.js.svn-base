...
//	{
		...
		{		
			...
			Constants :
			{
				ANCHOR_NONE :			0<<0,			///< No anchor, i.e. not relative-to-screen (0)
				ANCHOR_LEFT :			1<<0,			///< Anchor to the left edge (1)
				ANCHOR_RIGHT :			1<<1,			///< Anchor to the right edge (2)
				ANCHOR_BOTTOM :		    1<<2,			///< Anchor to the bottom edge (4)
				ANCHOR_TOP :			1<<3,			///< Anchor to the top edge (8)
				ANCHOR_CENTER_H :		1<<4,			///< Anchor to the horizontal center (16)
				ANCHOR_CENTER_V :		1<<5,			///< Anchor to the vertical center (32)

				ANCHOR_TL : 1<<0|1<<3,				///< Anchor to the Top-Left (9)
				ANCHOR_TC : 1<<3|1<<4,			///< Anchor to the Top-Center (24)
				ANCHOR_TR : 1<<3|1<<1,				///< Anchor to the Top-Right (10)
				ANCHOR_CL : 1<<5|1<<0,			///< Anchor to the Center-Left
				ANCHOR_CC : 1<<4|1<<5,		///< Anchor to the Center (48)
				ANCHOR_CR : 1<<5|1<<1,			///< Anchor to the Center-Right
				ANCHOR_BL : 1<<2|1<<0,				///< Anchor to the Bottom-Left (5)
				ANCHOR_BC : 1<<2|1<<4,			///< Anchor to the Bottom-Center
				ANCHOR_BR : 1<<2|1<<1,			///< Anchor to the Bottom-Right (6)

                FLAG_NONE :						0<<0,		///< No flag, all geometric transforms are considered
                FLAG_IGNORE_ROTATION :			1<<0,		///< ignore rotation of the geometry
                FLAG_IGNORE_ANIMATIONS :		1<<1,		///< ignore animations of the geometry
                FLAG_IGNORE_SCREEN_RESOLUTION :	1<<2,		///< same as FLAG_MATCH_DISPLAY
                FLAG_MATCH_DISPLAY:             1<<2,       ///< scale model to be same physical size on all displays
                FLAG_AUTOSCALE:				    1<<3        ///< Autoscale geometries according to the screen resolution and/or display density.
			},
...
arel.ClientInterface =
{
	...
	scene:
	{
		 ...
		 * e.g. <br />arel://scene/sharescreenshot
		 * @private 
		 */
		shareScreenshot : function()
		{
			...
			var request = arel.ClientInterface.protocol + this.sceneTerm + "/sharescreenshot";
			return arel.ClientInterface.out(request);
		},

        /** Initiate a continuous visual search
         * e.g. <br />arel://scene/requestvisualsearch/?database="test123"
         * @private
         */
        requestVisualSearch : function( _databaseName )
        {
            arel.Debug.logStream("Requesting visualsearch: " + _databaseName);

            var request = arel.ClientInterface.protocol + this.sceneTerm + "/requestvisualsearch/?database=" + _databaseName;
			return arel.ClientInterface.out(request);
        }
...
arel.Object = function()
{
	...
	this.setParameter = function(_key, _value, _override) {
		...
		if(this.parameters)
		{
			...
			if(this.parameters[_key] && !override)
				return false;
		...
		{
			arel.Debug.error("Error occured. Parameters missing");
			return false;
		}
...
{	
	 ...
	 * 
	 */
	startInstantTracking: function(_type)
	{
		return arel.ClientInterface.scene.startInstantTracking(arel.Util.toParameter({"value" : _type}));	
	},
	...
	shareScreenshot : function ()
	{
		return arel.ClientInterface.scene.shareScreenshot();
	},

    /** Triggers a continuous visual search request
     *
     * @requires AREL 2.1 (API level 9)
     * @param _databaseName the name of the database you want to query;
     * This will provide you a callback with type ONVISUALSEARCHRESULT
     */
    requestVisualSearch : function ( _databaseName )
    {
        return arel.ClientInterface.scene.requestVisualSearch(_databaseName);
    }

};

/**
 * Interface for manipulating the GestureHandler object
 * @type {Object}
 */
arel.GestureHandler =
{
    ...
    CATEGORY: arel.Config.GESTUREHANDLER_CATEGORY,
		 ...
		 * @param {string} arObjectID Object ID of the Object to be added
		 */
		 ...
		 * @param {string} arObjectID Object ID of the Object to be removed
		 */
...
arel.Events = 
{
	...
	Scene : 
	{
		 ...
		 * Event Callback Parameter: array of results or empty result if not successful
	...
	Media:
	{
		 ...
		 */		
		ONSOUNDPLAYBACKCOMPLETE: "onsoundplaybackcomplete",
	 ...
	 * @param {arel.Object|arel.Scene|string|arel.Media} eventIDOrElement arObject, scene or media url (path to image, sound, video or website) the event should be removed from or the eventID 
...
arel.Object.Model3D = function(_id, _modelPath, _texturePath)
{
	 ...
	 * @param {int} _speed The speed of the animation in fps. Default is 7fps. 
	 * @requires AREL 2.0 (API level 8)
	 */
	 ...
	 * Defines the animation speed of an object (only valid for md2/fbx models).
	 * @requires AREL 2.0 (API level 8)
	 * @return {int} The speed of the animation in fps. Default is 7fps. 
	 */
...
arel.Media = 
{
	 ...
	 * @param {boolean|string} external If set to true, the external browser will be opened, otherwise the internal.
	 */
	...
	/**
	 * Pauses the sound.
	 * @param {string} soundPath File path to the mp3 to be paused 
	 */
	...
	/**
	 * Stop the sound.
	 * @param {string} soundPath File path to the mp3 to be stopped
	 */
